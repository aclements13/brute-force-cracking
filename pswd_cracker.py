...
def md5sum(s):
    ...
    # string.
    #first changes the string to binary
    s_binary = s.encode()
...
def permutations(length, alphabet=ALPHABET):
    ...
    if length == 1 or length == 0:
        for letter in alphabet:
            yield letter
    #recursive case, call permutations, but with a smaller length
    else:
        for letter in alphabet:
...
def crack(hashes, length, alphabet=ALPHABET, prefix=''):
    ...
    #list comprehension with internal logic
...
def whack(arguments):
    ...
    #gets individual arguments from the tuple 
    hashes = arguments[0]
    length = arguments[1]
